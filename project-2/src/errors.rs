...
use std::fmt; // display mssg formatter
...
impl ToString for ErrorMessage { // enum --> string conversion
...
impl ErrorMessage {
    fn to_str(&self) -> String { // enum error --> error message mapping
        match self {
...
impl HttpError {
    ...
    pub fn into_http_response(self) -> Response { // HttpError -> HTTP ErrorResponse [that is readable]
