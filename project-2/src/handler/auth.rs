    ...
    match result {
        Ok(_user) => {
            ...
            Ok((
                ...
                Json(Response {
                    status: "success",
                    message: "Registration successful! Please verify your account".to_string(),
                }),
            ))
        }
...
pub async fn login(
    ...
    if password_matched {
        ...
        .map_err(|e| HttpError::server_error(e.to_string()))?; // creating jwt token

        let cookie_duration = time::Duration::minutes(app_state.env.jwt_maxage * 60);
        let cookie = Cookie::build(("token", token.clone()))
            .path("/")
            .max_age(cookie_duration)
            .http_only(true) // against CSRF
            .build();
        ...
        headers.append(header::SET_COOKIE, cookie.to_string().parse().unwrap()); // embedding cookie in the header

        let mut response = response.into_response();
        response.headers_mut().extend(headers); // embedding additional cookie header into default Response headers
...
pub async fn verify_email(
    ...
    query_params
        .validate()
        .map_err(|e| HttpError::bad_request(e.to_string()))?; // query params
    ...
    app_state
        ...
        .await
        .map_err(|e| HttpError::server_error(e.to_string()))?; // verifying the token in DB
...
pub async fn forgot_password(
    ...
    let response = Response {
        message: "Password reset link has been sent to your mail".to_string(),
        status: "success",
    };
...
pub async fn reset_password(
    ...
    let user_id = uuid::Uuid::parse_str(&user.id.to_string()).unwrap();
    let hash_password =
        password::hash(&body.new_password).map_err(|e| HttpError::server_error(e.to_string()))?; // new hashed password

    app_state
        .db_client
        .update_user_password(user_id.clone(), hash_password)
        .await
        .map_err(|e| HttpError::server_error(e.to_string()))?; // updating password

    app_state
        .db_client
        .verified_token(&body.token)
        .await
        .map_err(|e| HttpError::server_error(e.to_string()))?; // verifying auth token
