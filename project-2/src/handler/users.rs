    ...
    Router::new()
        .route(
            ...
            get(get_me).layer(middleware::from_fn(|state, req, next| {
                role_check(state, req, next, vec![UserRole::Admin, UserRole::User]) // both admin & user allowed
            })),
        )
        .route(
            "/users",
            get(get_users).layer(middleware::from_fn(|state, req, next| {
                role_check(state, req, next, vec![UserRole::Admin]) // only admin allowed
            })),
        )
...
pub async fn get_me(
    ...
    let filtered_user = FilterUserDto::filter_user(&user.user); // returns authenticated user from attached JWT auth headers middleware
...
pub async fn update_user_name(
    ...
    let user = &user.user;
    let user_id = uuid::Uuid::parse_str(&user.id.to_string()).unwrap(); // <string> user_id -> <UUID> user_id
